...
public:
    ...
    OrtSession* load(const std::string& path, bool nnapi = true,
        ...
        // Performance mode: Supertonic's vector_estimator is invoked repeatedly
        // (one ORT Run per flow-matching step). The general speech-core default
        // disables ORT's CPU arena and memory-pattern planner to reduce peak RSS
        // for very large multi-session models. That trade-off is poor for this
        // small, repeatedly-invoked TTS graph, so allow this session to opt back
        // into ORT's normal CPU memory reuse.
        if (performance_mode) {
            ort_check(api_, api_->EnableCpuMemArena(opts));
            ort_check(api_, api_->EnableMemPattern(opts));
            // Keep this session local to avoid cross-model arena contention.
            ort_check(api_, api_->AddSessionConfigEntry(
                opts, "session.use_env_allocators", "0"));
        } else {
        ...
        // General speech-core mode favors lower peak RSS. Supertonic passes
        // performance_mode=true above, so its repeated VE session keeps ORT's
        // arena + mem-pattern planner enabled instead of taking this low-memory
        // path.
        if (!performance_mode) {
